...
var eval = function(){
  ...
  hand = sortHand(hand);
  var x = 'whatevs';
  if(isStraightFlush(hand)){ x = 'Straight Flush'; }
  else if(isQuads(hand)){ x =  'Four of a Kind'; }
  else if(isBoat(hand)){ x =  'Full House'; }
//  else if(isFlush(hand)){ x =  'Flush'; }
  else if(isStraight(hand)){ x =  'Straight'; }
  else if(isTrips(hand)){x =  'Three of a Kind'; }
  else if(isTwoPair(hand)){x =  'Two Pair'; }
  else if(isPair(hand)){x = 'One Pair'; }
  else {x =  '' + hand[0].rank + ' high';};
